    ...
    public class SetupModule : IModule
    {
        ...
        private async Task<int> ChoosePoolAsync()
        {
            ...
            else
            {
                ...
                string workerName = await _data.Settings.GetWorkerNameAsync();

                chosenPool.SetWalletInfo(wallet, publicKey);

                chosenPool.SetWorkerName(workerName);
        ...
        private async Task<int> WorkerSetupAsync()
        {
            while (true)
            {
                string workerName = await _settings.GetWorkerNameAsync();

                SelectionPrompt<string> selectionPrompt = new SelectionPrompt<string>();
                selectionPrompt.WrapAround = true;

                selectionPrompt.Title($"Step: {_currentStep + 1}/{_steps.Count}\nSetup Worker Name. Current: {workerName ?? "??"}.");
        ...
        private async Task SetupWorkerName()
        {
            Guid id = Guid.NewGuid();

            TextPrompt<string> deviceNamePrompt = new TextPrompt<string>("Worker Name:");

            string defaultWorker = id.ToString();

            if (!String.IsNullOrEmpty(_settings.WorkerName))
            {
                defaultWorker = _settings.WorkerName;
            }

            deviceNamePrompt.DefaultValue(defaultWorker);
            string workerName = await deviceNamePrompt.ShowAsync(AnsiConsole.Console, _cts.Token);

            AnsiConsole.Clear();

            if (String.IsNullOrEmpty(workerName))
            {
                _settings.WorkerName = defaultWorker;
