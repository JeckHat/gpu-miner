...
{
    public enum BitzHQRequestTypes { Ready, Submission };
    public enum BitzHQResponseTypes { StartMining, SubmissionResult };

    public abstract class BitzHQPool : WebsocketPool
    {
        ...
        public override bool RequiresKeypair { get; } = false;
        public override Uri WebsocketUrl => new Uri($"wss://{HostName}/v2/ws-pubkey?pubkey={_publicKey}&timestamp={_timestamp}&worker_name={_workerName}");
        ...
        protected async Task DisplayOptionsAsync(CancellationToken token)
        {
            while (true)
            {
                ...
                foreach (Coin c in Enum.GetValues(typeof(Coin)))
                {
                    ...
                    builder.AppendLine($"BITZ Info:");
                    builder.AppendLine($"   {"Wallet Balance".PadRight(14)} {walletBalance} BITZ");
                    builder.AppendLine($"   {"Mining Rewards".PadRight(14)} {miningReward} BITZ " +
